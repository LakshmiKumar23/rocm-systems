...
@pytest.fixture
def julia_environment(rocprof_config) -> dict[str, str]:
...
class TestJulia(RocprofsysTest):
    ...
    # Goal: Verify that Julia programs can be profiled
    @pytest.mark.hpc
    @pytest.mark.gpu
    @pytest.mark.parametrize("mode", ["sys_run"])
    def test_vecadd(self, mode, julia_environment, rocprof_config):
        env = julia_environment.copy()
        if env is None:
            pytest.skip("Unable to resolve Julia library paths")
        result = self.run_test(mode, "vecadd.jl", env=julia_environment, pre_run_args=[str(rocprof_config.julia)])
