...
class TestJacobi(RocprofsysTest):
    ...
    # Goal: Verify zero-copy behavior
    @pytest.mark.openmp
    @pytest.mark.xnack
    ...
    def test_hip(self, mode, hpc_hip_environment):
        ...
        result = self.run_test(mode, target="jacobi-hip", env=env, run_args=self.hip_run_args, mpi_ranks=2)
        ...
        self.assert_perfetto(
            ...
            print_output=True,
            pass_regex=[rf"LocalLaplacianKernel.*\|\s+{MERGED_LAPLACIAN_KERNEL_COUNT}\s+\|"],
        )
...
class TestMatrixExponential(RocprofsysTest):
    ...
    def test(self, mode, hpc_hip_environment, matrix_exponential_rules):
        ...
        result = self.run_test(mode, target="matrix-exponential-streams-sync-hip", env=env)
        self.assert_regex(result,
...
class TestSplitCopyComputeHWQueues(RocprofsysTest):
    ...
    def test(self, mode, hpc_hip_environment, split_copy_compute_hw_queues_rules):
        ...
        result = self.run_test(mode, target="split-copy-compute-hw-queues", env=env, run_args=[str(self.nstreams)])
        self.assert_regex(result)
